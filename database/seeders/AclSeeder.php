<?php

namespace Database\Seeders;

use App\Models\Permission;
use App\Models\PermissionGroup;
use App\Models\Role;
use Illuminate\Database\Seeder;

class AclSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {

        $roles = [
            ['name' => 'Super Admin', 'guard_name' => 'web', 'is_active' => 1],
            ['name' => 'Admin', 'guard_name' => 'web', 'is_active' => 1],
            
        ];

        foreach ($roles as $role) {
            Role::updateOrCreate(['name' => $role['name']], $role);
        }


        $permissionGroups = [
            ['name' => 'Dashboard', 'order' => 1],
            ['name' => 'Users', 'order' => 2],
            ['name' => 'Bookings', 'order' => 3],
            ['name' => 'Vehicles', 'order' => 4],
            ['name' => 'Reviews', 'order' => 5],
            ['name' => 'Routes', 'order' => 6],
            ['name' => 'Attendance', 'order' => 7],
            ['name' => 'Roles', 'order' => 8],
            ['name' => 'Permission Groups', 'order' => 9],
            ['name' => 'Permissions', 'order' => 10],
            
        ];

        foreach ($permissionGroups as $group) {
            PermissionGroup::updateOrCreate(['name' => $group['name']], $group);
        }

        $permissions = [
            ['name' => 'View Dashboard', 'permission_group_id' => 1, 'guard_name' => 'web'],
            ['name' => 'View Dashboard Stats', 'permission_group_id' => 1, 'guard_name' => 'web'],
            ['name' => 'View Dashboard Analytics', 'permission_group_id' => 1, 'guard_name' => 'web'],
            ['name' => 'View Bookings Stats', 'permission_group_id' => 1, 'guard_name' => 'web'],
            ['name' => 'View Vehicles Stats', 'permission_group_id' => 1, 'guard_name' => 'web'],
            ['name' => 'View Recent Bookings', 'permission_group_id' => 1, 'guard_name' => 'web'],

            ['name' => 'Users List', 'permission_group_id' => 2, 'guard_name' => 'web'],
            ['name' => 'Create User', 'permission_group_id' => 2, 'guard_name' => 'web'],
            ['name' => 'View User', 'permission_group_id' => 2, 'guard_name' => 'web'],
            ['name' => 'Update User', 'permission_group_id' => 2, 'guard_name' => 'web'],
            ['name' => 'Delete User', 'permission_group_id' => 2, 'guard_name' => 'web'],

            ['name' => 'Bookings List', 'permission_group_id' => 3, 'guard_name' => 'web'],
            ['name' => 'Create Booking', 'permission_group_id' => 3, 'guard_name' => 'web'],
            ['name' => 'View Booking', 'permission_group_id' => 3, 'guard_name' => 'web'],
            ['name' => 'Update Booking', 'permission_group_id' => 3, 'guard_name' => 'web'],
            ['name' => 'Delete Booking', 'permission_group_id' => 3, 'guard_name' => 'web'],
            ['name' => 'Update Booking Status', 'permission_group_id' => 3, 'guard_name' => 'web'],
            ['name' => 'Update Route Status', 'permission_group_id' => 3, 'guard_name' => 'web'],
            ['name' => 'View Booking Voucher', 'permission_group_id' => 3, 'guard_name' => 'web'],

            ['name' => 'Vehicles List', 'permission_group_id' => 4, 'guard_name' => 'web'],
            ['name' => 'Create Vehicle', 'permission_group_id' => 4, 'guard_name' => 'web'],
            ['name' => 'View Vehicle', 'permission_group_id' => 4, 'guard_name' => 'web'],
            ['name' => 'Update Vehicle', 'permission_group_id' => 4, 'guard_name' => 'web'],
            ['name' => 'Delete Vehicle', 'permission_group_id' => 4, 'guard_name' => 'web'],
            ['name' => 'Update Vehicle Status', 'permission_group_id' => 4, 'guard_name' => 'web'],

            ['name' => 'Reviews List', 'permission_group_id' => 5, 'guard_name' => 'web'],
            ['name' => 'Create Review', 'permission_group_id' => 5, 'guard_name' => 'web'],
            ['name' => 'View Review', 'permission_group_id' => 5, 'guard_name' => 'web'],
            ['name' => 'Update Review', 'permission_group_id' => 5, 'guard_name' => 'web'],
            ['name' => 'Delete Review', 'permission_group_id' => 5, 'guard_name' => 'web'],
            ['name' => 'Update Review Status', 'permission_group_id' => 5, 'guard_name' => 'web'],

            ['name' => 'Routes List', 'permission_group_id' => 6, 'guard_name' => 'web'],
            ['name' => 'Create Route', 'permission_group_id' => 6, 'guard_name' => 'web'],
            ['name' => 'View Route', 'permission_group_id' => 6, 'guard_name' => 'web'],
            ['name' => 'Update Route', 'permission_group_id' => 6, 'guard_name' => 'web'],
            ['name' => 'Delete Route', 'permission_group_id' => 6, 'guard_name' => 'web'],
            ['name' => 'Update Route Status', 'permission_group_id' => 6, 'guard_name' => 'web'],

            ['name' => 'Attendance List', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'Create Attendance', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'Mark Attendance', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'View Attendance', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'Delete Attendance', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'Attendance Rule List', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'Create Attendance Rule', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'Update Attendance Rule', 'permission_group_id' => 7, 'guard_name' => 'web'],
            ['name' => 'Delete Attendance Rule', 'permission_group_id' => 7, 'guard_name' => 'web'],

            ['name' => 'Roles List', 'permission_group_id' => 8, 'guard_name' => 'web'],
            ['name' => 'Create Role', 'permission_group_id' => 8, 'guard_name' => 'web'],
            ['name' => 'View Role', 'permission_group_id' => 8, 'guard_name' => 'web'],
            ['name' => 'Update Role', 'permission_group_id' => 8, 'guard_name' => 'web'],
            ['name' => 'Delete Role', 'permission_group_id' => 8, 'guard_name' => 'web'],

            ['name' => 'Permission Groups List', 'permission_group_id' => 9, 'guard_name' => 'web'],
            ['name' => 'Create Permission Group', 'permission_group_id' => 9, 'guard_name' => 'web'],
            ['name' => 'View Permission Group', 'permission_group_id' => 9, 'guard_name' => 'web'],
            ['name' => 'Update Permission Group', 'permission_group_id' => 9, 'guard_name' => 'web'],
            ['name' => 'Delete Permission Group', 'permission_group_id' => 9, 'guard_name' => 'web'],

            ['name' => 'Permissions List', 'permission_group_id' => 10, 'guard_name' => 'web'],
            ['name' => 'Create Permission', 'permission_group_id' => 10, 'guard_name' => 'web'],
            ['name' => 'View Permission', 'permission_group_id' => 10, 'guard_name' => 'web'],
            ['name' => 'Update Permission', 'permission_group_id' => 10, 'guard_name' => 'web'],
            ['name' => 'Delete Permission', 'permission_group_id' => 10, 'guard_name' => 'web'],
     
        ];

        foreach ($permissions as $permission) {
            Permission::updateOrCreate(['name' => $permission['name']], $permission);
        }

        ### Admin Permissions
        $role = Role::find(2);
        $role->syncPermissions([
            'Users List',
            'Create User',
            'View User',
            'Update User',
            'Delete User',

        ]);

        ### Institution Permissions
        // $role = Role::find(3);
        // $role->syncPermissions([
            // 'Users List', 
        // ]);

    }
}
