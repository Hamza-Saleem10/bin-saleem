...
class UpdateInstitutionRequest extends FormRequest
{
    ...
    {
        $institutionId = $this->route('institution'); // Assuming route parameter is 'institution'

        return [
            // Institution
            'name' => 'sometimes|required|string|max:191',
            'level_2_id' => 'sometimes|required|exists:level_2,id', // district
            'level_3_id' => [
                'sometimes',
                'required',
            ...
            'institution_nature' => 'sometimes|required|in:Individual,Branch,Franchise',
            'institution_level' => 'sometimes|required|in:Primary,Elementary,High',
            'management_nature' => 'sometimes|required|in:Association of Person,Corporate Body,Educational Society Individual',
            'institution_type' => 'sometimes|required|in:School,ECD Centre,Day-care Centre,Tuition Centre',
            'teaching_level' => 'sometimes|required|in:Pre-Primary,Primary,Middle Secondary,High Secondary or equivalent',
            'institution_medium' => 'sometimes|required|in:English,Urdu,Both',
            'institution_gender' => 'sometimes|required|in:Boys Campus,Girls Campus,Co-Education',
            'address' => 'nullable|string|max:256',
            'latitude' => 'sometimes|required|numeric|between:-90,90',
            'longitude' => 'sometimes|required|numeric|between:-180,180',
            'institution_official_web_url' => 'sometimes|required|url|max:128',
            'institution_official_email' => 'sometimes|required|email|max:128',
            'institution_phone' => 'sometimes|required|string|max:20',
            'institution_fax' => 'nullable|string|max:20',
            'institution_public_email' => 'sometimes|required|email|max:30',
            'board_of_directors_owners' => 'sometimes|required|boolean',
            'board_of_directors' => 'sometimes|required_if:board_of_directors_owners,1|array',
            'board_of_directors.*.name' => 'sometimes|required_if:board_of_directors_owners,1|nullable|string|max:64',
            'board_of_directors.*.designation' => 'nullable|string|max:64',
            'male_faculty' => 'sometimes|required|integer|min:0',
            'female_faculty' => 'sometimes|required|integer|min:0',
            'total_faculty' => 'nullable|integer|min:0',
            'examination_board' => 'sometimes|required|in:BISE,CIE,PLC',

            // Types of Curriculum Adopted
            'curriculums' => 'sometimes|required|array',
            'curriculums.*' => ['nullable', Rule::in(['National Curriculum', 'CIE', 'PLC'])],

            'national_education_policy_adherence' => 'sometimes|required|string|max:191',
            'other_allied_facilities' => ['sometimes', 'required', Rule::in(['Yes', 'No'])],

            // Enrollment Counts
            'grade_id' => 'sometimes|required|exists:grades,id',
            'male_students' => 'sometimes|required|integer|min:0',
            'female_students' => 'sometimes|required|integer|min:0',

            // Owner
            'owner.name' => 'sometimes|required|string|max:64',
            'owner.designation' => 'sometimes|required|string|max:64',
            'owner.mobile' => 'sometimes|required|string|max:20',
            'owner.phone' => 'nullable|string|max:20',
            'owner.fax' => 'nullable|string|max:20',
            'owner.email' => 'sometimes|required|email|max:64',

            // Principal
            'principal.name' => 'sometimes|required|string|max:64',
            'principal.designation' => 'sometimes|required|string|max:64',
            'principal.mobile' => 'sometimes|required|string|max:20',
            'principal.phone' => 'nullable|string|max:20',
            'principal.fax' => 'nullable|string|max:20',
            'principal.email' => 'sometimes|required|email|max:64',

            // Faculty info (includes CV upload)
            'faculty' => 'nullable|array',
            'faculty.*.name' => 'sometimes|required_with:faculty|string|max:64',
            'faculty.*.cnic' => 'sometimes|required_with:faculty|string|max:15',
            ...
            'faculty.*.cv' => 'nullable|file|mimes:pdf,doc,docx|max:5120',

            // Infrastructure
            'infrastructure.building_type' => 'sometimes|required|in:Purpose Built,Residential,Commercial',
            'infrastructure.building_possession' => 'sometimes|required|in:Owned,Leased,Rented',
            'infrastructure.area_in_kanal' => 'sometimes|required|numeric|min:0',
            'infrastructure.area_in_marla' => 'sometimes|required|numeric|min:0',
            'infrastructure.no_of_classrooms' => 'sometimes|required|integer|min:0',
            ...
            'facilities.other_facilities' => 'sometimes|required_if:facilities.has_other,1|nullable|string|max:191',

            // Library
            'library.has_library' => 'sometimes|required|boolean',
            ...
            'other_resources.other_instructional_material' => 'sometimes|required|boolean',
            ...
            // Laboratories
            'science_laboratories' => 'sometimes|required|in:yes,no,1,0,true,false',
            'physic_laboratories' => 'sometimes|required_if:science_laboratories,yes,1|in:yes,no',
            'no_of_physic_laboratory_staff' => 'sometimes|required_if:physic_laboratories,yes|nullable|integer|min:0',
            ...
            'bio_laboratories' => 'sometimes|required_if:science_laboratories,yes,1|in:yes,no',
            'no_of_bio_laboratory_staff' => 'sometimes|required_if:bio_laboratories,yes|nullable|integer|min:0',
            ...
            'chemistry_laboratories' => 'sometimes|required_if:science_laboratories,yes,1|in:yes,no',
            'no_of_chemistry_laboratory_staff' => 'sometimes|required_if:chemistry_laboratories,yes|nullable|integer|min:0',
            ...
            'computer_laboratories' => 'sometimes|required|in:yes,no,1,0,true,false',
            'no_of_computers' => 'sometimes|required_if:computer_laboratories,yes,1|nullable|integer|min:0',
            ...
            'fee_structure_public' => 'sometimes|required|in:yes,no',
            'scholarship_policy_public' => 'sometimes|required|in:yes,no',
            'other_income_source' => 'sometimes|required|in:yes,no',
            'record_income_expenditure' => 'sometimes|required|in:yes,no',
            'financial_accounts_audited' => 'sometimes|required|in:yes,no',
            'academic_calendar_public' => 'sometimes|required|in:yes,no',
            'faculty_to_admin_staff_ratio' => 'sometimes|required|string|max:255',
            'extra_curricular_activities' => 'sometimes|required|string|max:500',
            'extracurricular_facilities' => 'sometimes|required|string|max:500',
        ];
    }
    ...
    public function after()
    {
        return [
            function ($validator) {
                // Only validate if male_faculty and female_faculty are present in the request
                if ($this->has('male_faculty') && $this->has('female_faculty') && $this->has('total_faculty')) {
